...
const CoursePage: NextPageWithLayout = () => {
  ...
  const [stakeRequiredCourseList, setStakeRequiredCourseList] = useState<
    Course[]
  >([]) // db 需要质押课程
  useEffect(() => {
  ...
  const userStakeCourseList = useMemo<(Course & { staked: boolean })[]>(() => {
    // db 和 链上合并的课程
    if (!userStakedCourseList)
      ...
      <div className="flex flex-col gap-4 mx-4">
        ...
        {!isConnected && (
          <div className="flex justify-center w-full">
            {' '}
            <ConnectButton label="Connect Wallet and Stake to Unlock" />
          </div>
        )}
        <div
